  ...
  <div class="py-5 px-3">
    <div class="lg:w-full   pr-2 pl-2 flex-col sm:flex-row sm:items-center  flex items-center mx-auto">
      ...
      <button onClick={()=> {
       ...
       class="flex-shrink-0 text-gray-500 rounded-custom-dansha border border-gray-400 py-2 px-8 focus:outline-none hover:bg-gradient-to-r from-blue-500 to-purple-500 hover:text-white hover:border-transparent  text-lg mt-10 sm:mt-0  transition-all">{userToken? `welcome ${userName} `:"Register Now"}</button>
    </div>
  </div>
</section>
{/* end head text ecommerc */}
   <section class="text-white py-5 body-font">
  <div class="container px-11 py-3 mx-auto">
  {loading? <LoadingTwo/>:  <div class="grid grid-cols-4 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2  max-sm:grid-cols-1 gap-3  -m-4">

{products?.data?.map((Producet)=> (
  <div key={Producet._id} class=" p-4 w-full rounded shadow-md">
